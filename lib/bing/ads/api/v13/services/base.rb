          ...
          class Base
                ...
                if fault.dig(:detail, :api_fault_detail)
                  ...
                  if retries_made < retry_attempts
                    ...
                    raise Bing::Ads::API::Errors::UnhandledSOAPFault,
                          "SOAP error (#{fault.keys.join(', ')}) while calling #{operation}. #{e.message}"
                  end
                end
            ...
            def handle_soap_fault(operation, fault_detail, key)
              if fault_detail[key][:errors] &&
                ...
                message = "SOAP error (#{fault_detail[key]}) while calling #{operation}."
                error = Bing::Ads::API::Errors::UnhandledSOAPFault.new(message, fault_detail[key])
                raise error
              end
            end
          end
